...
import { CloudRain, Loader2, Play, TrendingUp, Zap } from "lucide-react";

import { api, type ForecastResult } from "@/lib/api";
import { cn, fmtNum } from "@/lib/utils";
import { SeverityBadge } from "./severity-badge";

type Props = { cuencaId: string; municipalityId?: string };
...
export function ForecastCard({ cuencaId, municipalityId }: Props) {
  ...
  async function forceDemoAlert() {
    if (!municipalityId) return;
    setForcing(true);
    setResult(null);
    try {
      const r = await api.fireTestAlert(municipalityId);
      if (r.status === "success") {
        setResult("🚨 Alerta EXTREMA del huaico 2017 enviada al canal Telegram");
      } else {
        setResult(`✖ ${r.error ?? "fallo"}`);
      }
    } catch (err) {
  ...
  return (
    <div className={cn("rounded-lg border p-4 space-y-3", SEVERITY_BG[sev] || SEVERITY_BG.low)}>
      ...
      <div className="space-y-2">
        ...
        {municipalityId && (
          <button
            onClick={forceDemoAlert}
            disabled={analyzing || forcing}
            className={cn(
              "w-full inline-flex items-center justify-center gap-1.5 rounded-md border border-red-500/40 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300",
          ...
          >
            ...
            {forcing ? "Disparando..." : "Simular evento extremo (huaico 2017)"}
          </button>
        )}
      </div>
      ...
      <div className="text-[10px] text-white/40 leading-relaxed">
        ...
        El boton rojo simula un evento extremo con datos reales del huaico de marzo 2017.
      </div>
    </div>
  );
}
